...
public function updateRecord($notrack = 0) {
	...
	$our_title = $db->quote($this->_title);
	$our_alternate_title = $db->quote($this->_alternate_title);
	$our_prefix = $db->quote($this->_prefix);

	$title_count = $db->query("SELECT title_id FROM title where title = $our_title");
	
	$qUpTitle = "UPDATE title SET title = $our_title , alternate_title = $our_alternate_title , description = " . $this->_description . ", pre = $our_prefix WHERE title_id = $title_count)";
...
function modifyLocation() {
	...
	foreach ($this->_location_id as $key => $value) {
		...
		$call_number = $db->quote($this->_call_number[$key]);
	    $location_count = $db->query("SELECT COUNT(*) FROM location WHERE call_number = $call_number");
...
$current_location_id = $db->last_id();
	...
	$current_location_id = $this->_location_id[$key];
	$this->_debug .= "<p>current loc id = $current_location_id";
}
      // If/else over, now do an insert to location_title
	$current_location_id = $this->_location_id[$key];

$qInsertLocTitle = "INSERT INTO location_title (title_id, location_id) VALUES ( '$this->_title_id[$key]' , '$current_location_id' )";
$this->_debug .= "<p>6. insert into location_title: $qInsertLocTitle</p>";
;
$rInsertLocTitle = $db->exec($qInsertLocTitle);

if (!$rInsertLocTitle) {
	echo blunDer("$current_location_id We have a problem with the insert location_title query: $qInsertLocTitle");
}
}
}
