...
class DbHandler {
	...
	function writeTable($qualifier, $subject_id = '', $description_search = 0) {
		...
		default:
			...
			$condition2 = "WHERE alternate_title LIKE " . $db->quote("%" + "J" + "%");
			
		}
		$first_letter = $qualifier;

		$qualifier = $db->quote("%$qualifier%");
		...
		$alternate_title_query = "SELECT distinct left(t.alternate_title,1) as initial, t.alternate_title as newtitle, t.description, location, access_restrictions, t.title_id as this_record,eres_display, display_note, pre, citation_guide, ctags, helpguide
			...
			INNER JOIN source as s
			ON rk.source_id = s.source_id
			WHERE alternate_title LIKE $qualifier
			AND eres_display = 'Y'
			ORDER BY newtitle";
		...
		// Merge the two results, alternate and main
		$main_alternate_titles = array_merge($main_titles,$alternate_titles);
		
		// Check to see if the qualifier is not a single letter like "C", it could be "All" or "Audio"
		if(strlen($first_letter) === 1) {
			
			// If it is a single letter, remove titles that don't start with that same letter
		foreach ($main_alternate_titles as $index => $title) {
			
			$title_first_letter = substr($title[0], 0);
			
			if ($title_first_letter != $first_letter) {
